...
//		routing.joinCheck();
	...
	protected TableRep transferBindings(int predId) {
		...
		transferListener(predId); //calling method here in order to transfer Listener 
		return db;
	}
	...
	public void join(String uri) throws Failed, Invalid {
		...
		UriBuilder uBuilder = UriBuilder.fromPath(uri); 
		URI ub = uBuilder.build(); 
	...
	public EventOutput listenForBindings(int id, String key) {
		...
		EventOutput eo = new EventOutput();     //may have to change this code changed code used to return null                        FIXME
		state.addListener(id, key, eo);
	...
	public void stopListening(int id, String key) {
		...
		state.removeListener(id, key); //may have to change code                                                                       FIXME

	}
	
	// added method for testing to move listener to new bindings                 FIXME 
	public void transferListener(int predId){
		
		//Map<String,SseBroadcaster> listenersMap = state.getListeners();
		//for(Entry<String,SseBroadcaster> entry : listenersMap.entrySet()) {
		
		synchronized(this) {
			Map<Integer,Map<String,EventOutput>> outputMaps = state.getOutputs();
			for(Entry<Integer,Map<String,EventOutput>> entry : outputMaps.entrySet()) {
				if((entry.getValue()) == null || entry.getValue().isEmpty()) { 
				 info("getting through first loop"); 
					continue; 
				}
				try { 
				takeOutListener(predId, entry.getValue(), entry.getKey());
			}catch( Exception e) { 
				System.out.println("error taking out listener");
			}
		}
	}
}
	public void takeOutListener (int predId, Map<String,EventOutput> entry, int removeKey) {
		
		for(String key : entry.keySet()) {
			OutboundEvent.Builder eb = new OutboundEvent.Builder();
			OutboundEvent event = eb.name("transfer-listener")
					            .id("-1")
					            .data(String.class, key)
					            .build();
			
			if(key != null && predId<0 || !DHTBase.inInterval(DHTBase.NodeKey(key), predId, info.id)) {
				info("In takeOutListener method"); 
				try {
					SseBroadcaster broadcaster = state.getListeners().get(key);
					if (broadcaster != null) {
					broadcaster.broadcast(event);
					}
					info("broadcast transfer complete"); 
					state.removeListener(removeKey, key);
				} catch (Exception e) {
					e.printStackTrace();
					info("failed getting/removing listener in takeOutListener method"); 
				}
			} 
		}
	}
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		 ...
		 */
		// might have to fix code added between lines 694 and 702                                                                     FIXME
		int id = NodeKey(key); 
		NodeInfo nodeKey = findSuccessor(id); 
		NodeInfo currNode = getNodeInfo(); 
		
		EventSource es = client.listenForBindings(nodeKey, currNode.id, key);     
	...
	public void listenOff(String key) throws DHTBase.Failed {
		 ...
		 */
        // might have to fix code                                                                                                     FIXME
		int id = NodeKey(key); 
		NodeInfo nodeKey = findSuccessor(id); 
		NodeInfo currNode = getNodeInfo(); 
		
		client.listenOff(nodeKey, currNode.id, key);
