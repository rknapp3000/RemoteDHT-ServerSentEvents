	...
	protected EventListener listener(final String key) {
		return new EventListener() {
	        ...
	        public void onEvent(InboundEvent ev) {
	         ...
	         //   msgln("in callback" + ev.getId() + ev.getName()); 
	         //testing code below   FIXME
	        	if (ev.getId().equals("-1")) {
					msgln(String.format("** Binding event (%s): Binding for the key %s is moved from Node", ev.getName(), key,
							ev.readData(String.class)));
					try {
					TimeUnit.SECONDS.sleep(5); //might have to take out
