...
public class State implements IState, IRouting {
	...
	public void addListener(int id, String key, EventOutput os) {
		...
		//may have to fix code from line 375 to line 388 											    FIXME
		...
		//recordOutput(id, key, os);   might have to use this code instead
	...
	public void removeListener(int id, String key) {
		...
		//may have to fix code from 298 to 308                				FIXME
		if(listeners.get(key)!=null) {
			EventOutput eo = outputs.get(id).get(key);
			try {
				eo.close();
			} catch(IOException e) {
				throw new RuntimeException("closing event output causing error", e);
			}
			
			outputs.get(id).remove(key);
			listeners.get(key).remove(eo);
		}
	...
	private void broadcastAddition(String key, String value) {
		...
		// may have to fix this method code from 315 to 323 													   FIXME
	...
	//added below methods for DHT.java methods testing transfer listener for new bindings FIXME
	...
	public void removeCallback(String key) {
		...
		//may have to change the three lines of code below 														FIXME
